...
class Model_volunteer extends CI_Model
{
    ...
    {
        return $this->db->select('*')->from('temp_add_ques')->where('q_id', $q_id)->order_by('id','DESC')->get()->result_array();
    }
    ...
    function prof_approve_question($id)
    {
        ...
        if (count($query_quiz) <= 1) {
           ...
           $approved_subquestion['user_id'] = $approved_question['user_id'];
           //print_r($approved_subquestion['user_id']);
          // $approved_subquestion['id'] =   $approved_question['id'];
            $approved_subquestion['q_id'] = $approved_question['q_id'];
            $approved_subquestion['question'] = $approved_question['question'];
            $approved_subquestion['answer'] = $approved_question['answer'];
            $approved_subquestion['explanation']=$approved_question['explanation'];
            $approved_subquestion['q_type'] = 2;
            if (count($query_quiz) == 1) {
               // $approved_question['q_id'] = $query_quiz['0']['q_id'];
                 //          $approved_subquestion['id']= $approved_question['q_id'];
            ...
            return $this->db->insert('add_quiz', $approved_subquestion);
        } else {
    ...
    public function get_resources($where) {
		return $this->quizdb->select('*')->from('other_study2')->where('add_by',$where)->order_by('id','DESC')->limit('50', '0')->get()->result_array();
    }
    
       function prof_subview_question($type)
    {
        return $this->db->select('*')->from('temp_add_ques')->join('temp_new_quiz', 'temp_new_quiz.q_id = temp_add_ques.q_id')->limit(50,0)->where('temp_add_ques.type', $type)->where('explanation IS NOT NULL')->get()->result_array();
    }

    public function temp_quiz(){
        //$this->load->database('Intellify');
$query = $this->db->query('SHOW CREATE TABLE add_quiz');
$queryResultArray = $query->result_array();
print_r( $queryResultArray[0]['Create Table'] );
    //$fields = $this->db->field_data('temp_new_quiz');
    //return $fields;
